...
void AssetImporter::AssetComplete(AssetImporter* obj, Il2CppObject* asyncOp)
{
    ...
    log(INFO, "AssetComplete is done");
    RET_V_UNLESS(CheckAssetClass(obj->asset));
    onLoadAssetComplete(obj->asset);
    if (obj->whenDone) {
        obj->whenDone(obj);
    }
}

bool AssetImporter::LoadAsset(UnityAssetLoader_OnLoadAssetCompleteFunction* onLoadAssetCompleteCall,std::string_view assetNameS)
{
    ...
    auto action = RET_0_UNLESS(il2cpp_utils::MakeAction(method, 0, this, AssetComplete));
...
void AssetImporter::AssetBundleComplete(AssetImporter* obj, Il2CppObject* asyncOp)
{
    ...
    log(DEBUG, "AssetBundleComplete");
    if (obj->permissionToLoadAsset)
    log(DEBUG, "permissionToLoadAsset");
        obj->LoadAsset(onLoadAssetComplete);
}

bool AssetImporter::LoadAssetBundle( UnityAssetLoader_OnLoadAssetCompleteFunction* onLoadAssetCompleteCall, bool alsoLoadAsset)
{
    ...
    if (!pathExists) {
        ...
        log(DEBUG, "Checking path of file");
    }
